...
public class ServiceOeuvre implements CRUD_oeuvre<Oeuvre> {
    ...
    @Override
    public List<Oeuvre> trie() throws SQLException {
        List<Oeuvre> oeuv =new ArrayList<>();
        String req = "SELECT * FROM oeuvre,panier where oeuvre.id_oeuvre=panier.id_oeuvre ORDER BY description";
    ...
    public Connection getConnection() throws SQLException {
     ...
     /*   String url = "jdbc:mysql://localhost:3306/piwj_sj";
        String user = "root";
        String password = "";
    ...
    public List<Oeuvre> selectByname(String ch) throws SQLException {
        List<Oeuvre> oeuv = new ArrayList<>();
        String req = "SELECT * FROM oeuvre,panier WHERE oeuvre.id_oeuvre=panier.id_oeuvre AND titre LIKE ?";
    ...
    public ServiceOeuvre() {
        ...
        User user = new User(22,"jhonDoe","1234567","john@example.com","password123","1990-01-01","Male","Regular");
    @Override
    public void insertOne(Oeuvre oeuvre) throws SQLException {
        String sql = "INSERT INTO `oeuvre` (`titre`,`description`,`prix`,`date`,`status`,`lienImg`,`id_cat`) VALUES "
        +"(?,?,?,?,?,?,?)";
        ...
        try {
            ...
            System.out.println("rr :"+oeuvre.getDescription());
            ...
            String substring2 = oeuvre.getLienImg().substring(15);
            statement.setString(6, substring2);
    ...
    @Override
    public List<Oeuvre> selectMYAll()throws SQLException {
        List<Oeuvre> OeuvreList=new ArrayList<>();
        String req = "SELECT * FROM `oeuvre` where idUser=?";
        PreparedStatement statement = cnx.prepareStatement(req);
    ...
    public List<Oeuvre> selectAllOeuvre()throws SQLException {
        List<Oeuvre> OeuvreList=new ArrayList<>();
        String req = "SELECT * FROM oeuvre,panier where (oeuvre.id_oeuvre=panier.id_oeuvre AND id_user = ?)";
        PreparedStatement statement = cnx.prepareStatement(req);
        statement.setInt(1,  user_id);
        ResultSet rs = statement.executeQuery(); // Use the prepared statement here
        while (rs.next()){
...
@Override
    ...
    public void deleteOne(int id) throws SQLException {
        ...
        try (PreparedStatement statement = cnx.prepareStatement(sql)) {
            ...
            statement.executeUpdate();
            System.out.println(sql);
            int rowsDeleted = statement.executeUpdate(); // Première et unique exécution de executeUpdate()
            if (rowsDeleted > 0) {
...
//    }
    ...
    public List<Oeuvre> searchByTitre(String titre) throws SQLException {
        List<Oeuvre> oeuvres = new ArrayList<>();
        String query = "SELECT * FROM oeuvre WHERE titre LIKE ? id_user = ?";

        try (Connection connection = getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            preparedStatement.setString(1,  titre + "%");
            preparedStatement.setInt(2,  user_id);// Search for titre containing the given string
